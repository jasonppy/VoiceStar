    ...
    def generate(
        ...
        if getattr(self.model_args, "y_sep_token", None) != None:
            encoded_frames = torch.cat(
                [
                    encoded_frames,
                    torch.LongTensor([self.model_args.y_sep_token] * self.model_args.n_codebooks)
                    .unsqueeze(0)
                    .unsqueeze(2)
        ...
        if isinstance(target_text, list):
            text_tokens = [self.phn2num[phn] for phn in target_text if phn in self.phn2num]
        else:
            text_tokens = [
                self.phn2num[phn]
        ...
        if prefix_transcript is not None:
            if isinstance(prefix_transcript, list):
                prefix_tokens = [
                    self.phn2num[phn] for phn in prefix_transcript if phn in self.phn2num
                ]
            else:
                prefix_tokens = [
                    self.phn2num[phn]
                    for phn in tokenize_text(self.text_tokenizer, text=prefix_transcript.strip())
                    if phn in self.phn2num
                ]
            ...
            if getattr(self.model_args, "x_sep_token", None) != None:
                text_tokens = (
                    prefix_tokens + [getattr(self.model_args, "x_sep_token", None)] + text_tokens
                )
            else:
        ...
        concat_frames, gen_frames = self.model.inference_tts(
            ...
            original_audio[..., : self.model_args.n_codebooks].to(self.device),  # [1,T,8]
