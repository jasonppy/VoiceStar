...
def cut_sequence(task):
    ...
    for item in metadata:
        ...
        out_phn_fn = os.path.join(
            output_dir, "phoneme", out_fn.replace(".flac", ".txt")
        )
        save_audio(
            data[int(item["vad"][0] * samplerate) : int(item["vad"][1] * samplerate)],
            out_audio_fn,
        )
...
def cut(split, audio_dir, mani_dir, output_dir, n_process=32, percent=0.5):
    split2manifest = {
        "train": [
        ...
        for mani_fn in split2manifest[split]:
            ...
            for item in data:
                ...
                if os.path.isfile(os.path.join(audio_dir, recording_id)):
                    ...
                    file_id = (
                        file_id.replace(".flac", "")
                        + f"_{vad[0]:.2f}_{vad[1]:.2f}.flac"
                    )
    ...
    tasks = [
        (
            os.path.join(audio_dir, recording_id),
            output_dir,
    ...
    print(
        f"take only {percent*100:.2f}% of the tasks, {len(tasks)} out of {ntasks} tasks"
    )
    #### take only a subet of tasks
    ...
    with multiprocessing.Pool(processes=n_process) as pool:
        for phns in tqdm.tqdm(
            pool.imap_unordered(cut_sequence, tasks), total=len(tasks)
        ):
            cnt += 1
            if phns != None:
...
def parse_args():
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="/data/scratch/pyp/datasets/librilight/librilight_example_preprocessed",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="take only this percent of the tasks, randomly sampled from each speaker",
    )
