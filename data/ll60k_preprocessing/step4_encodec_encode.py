...
def parse_args():
    parser = argparse.ArgumentParser(
        description="encode the librilight dataset using codec model"
    )
    parser.add_argument(
        "--dir",
        type=str,
    ...
    class mydataset(torch.utils.data.Dataset):
        def __init__(self, split):
            ...
            self.data = [
                l
                for l in self.data
                if os.path.isfile(os.path.join(self.audio_dir, l[0]))
            ]
    ...
    with open(mani_fn, "w") as mani_wf:
        ...
        for m, mega_batch in enumerate(tqdm.tqdm(loader)):
            ...
            try:
                ...
                if (
                    len(lengths) == 0
                ):  # the loader might not find any audio because step3 will write to manifest first, and then might selection a subset to cut and save audio
                    continue
